...
workspace_id=$(az monitor log-analytics workspace show --resource-group $RESOURCE_GROUP --workspace-name $RESOURCE_GROUP --query customerId -o tsv)
...
## removoed flag "--internal-only" 
az containerapp env create --name $CONTAINERAPPS_ENVIRONMENT --resource-group $RESOURCE_GROUP --location "$LOCATION" --infrastructure-subnet-resource-id $INFRASTRUCTURE_SUBNET  --infrastructure-resource-group $MANAGED_RG
